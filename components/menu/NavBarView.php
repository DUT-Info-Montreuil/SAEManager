...
class NavBarView extends GenericComponentView
{
	...
	{
		$this->affichage = '<div class="menu d-flex text-white">';
		$this->addMenuToggle();
		$this->addBrand();
		$this->addLinks();
		$this->addUserSection();
	...
	private function addMenuToggle()
	{
...
HTML;
	...
	private function addUserSection()
	{
		if (isset($_SESSION['loginUtilisateur'])) {
			$username = htmlspecialchars($_SESSION['loginUtilisateur']);
			$this->affichage .= <<<HTML
            <div class="d-flex">
...
HTML;
	...
	private function addSideMenu()
	{
		$module = $_GET['module'] ?? '';
		$action = $_GET['action'] ?? '';
		$saeID = htmlspecialchars($_GET['id'] ?? '');

		$this->affichage .= '<div id="side-menu" class="side-menu"><ul>';
